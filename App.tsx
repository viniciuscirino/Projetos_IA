...
// FIX: Removed conflicting global declaration.
// The global window interface is now correctly typed and centralized in `types.ts`.
...
const App: React.FC = () => {
  ...
  const handleSave = useCallback(async () => {
    try {
        const result = await sqliteService.backupDatabaseToFile();
        if (result.success) {
            toastService.show('success', 'Backup Concluído', 'O arquivo de backup foi gerado com sucesso.');
        } else if (result.message && !result.message.includes('cancelado')) {
            toastService.show('error', 'Erro de Backup', result.message);
        }
    } catch (error) {
